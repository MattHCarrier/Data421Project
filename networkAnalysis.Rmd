...
```{r}
g86<-graph_from_data_frame(df86)
...
```{r}
g86$totalExport<-totalExport86
g96$totalExport<-totalExport96
g06$totalExport<-totalExport06
g16$totalExport<-totalExport16
...
V(g86)[degree(g86)>=max(degree(g86)/1)]
...
(subset(group,c(g86$community)==i))
...
go06$totalExport<-totalExport06
n<-max(g06$community)
group2<-data.frame(c(g06$community))

for (i in 1:n){
df<-(subset(group2,c(g06$community)==i))
}
df[]
```
...
group3TotalExport<-data.frame(c(g16$totalExport))
for (i in 1:n){

df[i]<-(subset(group3,c(g16$community)==i))
}
df[2]
```
```{r}
